package main

import (
	"bytes"
	"encoding/binary"
	"fmt"
	"lib"
	"net"
	"unsafe"
)

type PacketHeader struct {
	Len         uint16   /*整包长度，包括包头+包体，但不包含自身的两个字节*/
	Start       [2]uint8 /*消息开始标记*/
	Main_ver    uint8    /*协议主版本号*/
	Sub_ver     uint8    /*协议子版本号*/
	Main_cmd    uint16   /*主命令字*/
	Code        uint8    /*校验code*/
	Sub_cmd     uint16   /*子命令字*/
	Seq         uint16   /*序列号 首次就填0*/
	Source_type uint8    /*消息来源 */
}

type Packet struct {
	Header PacketHeader
	Buffer [8000]byte
	Length int32
	Pos    int32
}

func PackString() {

}

func UnpackString() {

}

func PackInt() {

}

// struct init begin
var in_packet, out_packet Packet

//in_packet.header = Packet{}
//in_packet.buffer = make([]byte, 80000)

//Packet{header:PacketHeader{}, buffer: make([]byte, 8000, 8000), Length:0, pos:0 }
// struct init end

var SendMap = [256]byte{}

var RecvMap = [256]byte{
	}

func main() {
	fmt.Println("Starting the server ...")
	lib.Version()
	fmt.Printf("sizeof: %v\n", unsafe.Sizeof(PacketHeader{}))

	// create listener:
	listener, err := net.Listen("tcp", "localhost:4444")
	if err != nil {
		fmt.Println("Error listening", err.Error())
		return // terminate program
	}
	// listen and accept connections from clients:
	for {
		conn, err := listener.Accept()
		if err != nil {
			fmt.Println("Error accepting", err.Error())
			return // terminate program
		}
		go doServerStuff(conn)
	}
}

func cat() {
	fmt.Print()

}

func doServerStuff(conn net.Conn) {
	for {
		buf := make([]byte, 512)
		//buf := new(bytes.Buffer)

		n, err := conn.Read(buf)

		fmt.Printf("Read: %x\n", buf)

		if err != nil {
			fmt.Println("Error reading", err.Error())
			return // terminate program
		}

		fmt.Printf("recv: %d\n", n)

		if n >= 14 {
			if GetHeader(buf[:14], n) {

				GetPacket(buf, n)

				Decode(in_packet.Buffer[14:], 17)

				ParseCmd(in_packet.Buffer[14:], 17)
			}
		}

		fmt.Printf("Received data: %x\n", string(buf[:31]))
		fmt.Printf("Decode data: %x\n", string(in_packet.Buffer[:31]))
	}
}

func log_debug(format string, v ...interface{}) {
	fmt.Printf(format, v...)
}

func GetHeader(buff []byte, rLen int) bool {
	log_debug("----------GetHeader begin -----------------\n")

	bb := bytes.NewBuffer(buff)

	var header PacketHeader

	err := binary.Read(bb, binary.BigEndian, &header)

	// for i := 0; i < rLen; i++ {
	// 	cmd[i] = buff[i]
	// }
	if err == nil {
		//fmt.Printf("CMD: %v\n", header.Pack_len)
		PrintHeader(&header)

		return true
	} else {
		fmt.Printf("err: %v\n", err)
		return false
	}

	log_debug("----------GetHeader end -----------------\n")

	return false
}

func PrintHeader(h *PacketHeader) {
	fmt.Printf("Length: %v\n", h.Len)
	log_debug("Start: %s\n", h.Start)
	log_debug("Main_cmd: %v\n", h.Main_cmd)
}

func Decode(buff []byte, n int) {
	fmt.Printf("org: %x\n", buff[:n])

	for i := 0; i < n; i++ {
		fmt.Printf("decode: %x\n", RecvMap[buff[i]])
		buff[i] = RecvMap[buff[i]]
	}

	fmt.Printf("proc: %x\n", buff[:n])
}

func Encode(buff []byte, n int) {

}

func ParseCmd(buff []byte, n int) {
	fmt.Printf("1: %d\n", byte2int32(buff[:4], 4))
	fmt.Printf("2: %d\n", byte2int16(buff[4:6], 2))
	fmt.Printf("3: %d\n", byte2int16(buff[6:8], 2))
	fmt.Printf("4: %d\n", byte2int32(buff[8:12], 4))
	fmt.Printf("5: %s\n", string(buff[12:16]))
}

func byte2int32(buff []byte, l int32) int32 {
	var result int32

	b := bytes.NewBuffer(buff[:l])

	error := binary.Read(b, binary.BigEndian, &result)

	if error == nil {
	}

	return result
}

func byte2int16(buff []byte, l int16) int16 {
	var result int16

	b := bytes.NewBuffer(buff[:l])

	error := binary.Read(b, binary.BigEndian, &result)

	if error == nil {
	}

	return result
}

func i2b(n int32, buff []byte) {

}

func GetPacket(buff []byte, n int) {
	log_debug("----------GetPacket begin -----------------\n")
	for i := 0; i < n; i++ {
		in_packet.Buffer[i] = buff[i]
	}

	in_packet.Pos = int32(n)
	in_packet.Length = int32(n)

	fmt.Printf("in_packet: %x\n", in_packet.Buffer[:n])
	log_debug("----------GetPacket end -----------------\n")
}
